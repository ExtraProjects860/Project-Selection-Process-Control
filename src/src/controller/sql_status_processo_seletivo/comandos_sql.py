...
SQL_QUANTIDADE_STATUS_PROCESSO_SELETIVO: str = """
    SELECT COUNT(id_status_processo_seletivo)
    FROM status_processo_seletivo;
"""


SQL_PEGAR_STATUS_PROCESSO_SELETIVO: str = """
    SELECT sps.id_status_processo_seletivo, sps.data_entrevista, sps.data_conclusao, du.nome_usuario, du.telefone, u.email, c.nome_cargo, e.nome_etapa, s.nome_setor, sps.perfil, sps.status_processo, sps.observacao, sps.forms_respondido, sps.avaliacao_forms
    ...
    INNER JOIN inscricao i ON i.id_inscricao = sps.id_status_processo_seletivo
